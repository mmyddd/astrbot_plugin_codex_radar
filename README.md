...
| `雷达历史 <模型>` | 只输出指定模型的近 72h 全思考强度历史（曲线图 + 各强度走势）。支持别名：`sol` / `terra` / `luna` / `5.5` / `d4flash`（deepseek-v4-flash），也支持完整模型名或唯一子串；无法唯一匹配时返回可用模型列表。 |
...
| 数据 | 接口 | 说明 |
|------|------|------|
| 智力效率 | `https://codexradar.com/api/intelligence-efficiency` | schema=1，含 `combos`（模型×强度）、`tasks`（评测题）、`cells`（每题每次运行的判分/耗时/花费） |
| 智力效率（备用） | `https://codexradar.com/data/intelligence-efficiency.json` | 站点前端内置的静态快照，主接口失败时自动回退 |
...
- **散点图坐标**：与站点一致，x 轴为相对综合成本指数（对数刻度；第二小值 ≥ 4× 最小值时启用断轴，避免大部分点挤在左侧），y 轴为 IQ 分数（0-150，上限取站点 niceMax 整点）；同一模型的各思考强度按 low→ultra 连线，点形状区分强度
...
- `deepseek-v4-flash` 系列的 IQ 历史当前为空（站点未产生样本），图中会跳过、文本标注「暂无数据」。
